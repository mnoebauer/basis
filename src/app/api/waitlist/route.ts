...
async function sendWaitlistConfirmationEmail(email: string) {
  ...
  if (!apiKey) {
    // Do not fail local/dev builds when email provider credentials are not configured.
    console.warn("Waitlist confirmation email skipped: RESEND_API_KEY is not set.");
    return;
  }
